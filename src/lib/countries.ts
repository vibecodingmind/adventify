...
export const popularCountryCodes = [
  'US', 'GB', 'CA', 'AU', 'NG', 'KE', 'ZA', 'BR', 'IN', 'PH',
  'DE', 'FR', 'JP', 'KR', 'MX', 'AR', 'CO', 'GH', 'TZ', 'UG',
];
...
/** Build the ordered countries list: popular first, then alphabetical rest */
export function getOrderedCountries(): Country[] {
  const popular = allCountries.filter((c) =>
    popularCountryCodes.includes(c.code)
  );
  const rest = allCountries.filter(
    (c) => !popularCountryCodes.includes(c.code)
  );
  return [...popular, ...rest];
}

export const COUNTRIES = getOrderedCountries();
